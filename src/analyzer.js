...
// Константы: путь к директории с образцами HTML и искомая подстрока (регистронезависимая)
const path = require('path');
const fs = require('fs');

// Ищем файлы в папке samples на уровень выше текущего файла
const FS_ROOT = path.join(__dirname, '..', 'samples');
...
function main() {
    if (!fs.existsSync(FS_ROOT)) {
        console.log(`Папка с образцами не найдена: ${FS_ROOT}`);
        return;
    }
